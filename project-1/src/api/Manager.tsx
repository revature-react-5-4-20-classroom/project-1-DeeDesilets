...
const manager = axios.create({
  ...
  export async function getReimbursementsByAUID (id : number) : Promise <Reimbursement[]> {
    ...
    const response = await manager.get('/reimbursements/author/userId');
  ...
  export async function submitReimbursements (reimbursementId = 0, author: number, amount: number, datesubmitted: number, description: string, type: number ) : Promise <Reimbursement> {
      ...
      try {
        const response : Reimbursement = await manager.post('/reimbursements', {"reimbursementid" : 0, "author" : "author" , "amount" : "amount", "datesubmitted" : "datesubmitted", "description" : "description", "type" : "type"});
  ...
  export async function getUserById() : Promise<User> {
    ...
    const response = await manager.get('/users');
  ...
  export async function checkingCredentials (un: string, pw: string): Promise<User> {
    try {
      const response = await manager.post('/login', {username: un, password: pw});
